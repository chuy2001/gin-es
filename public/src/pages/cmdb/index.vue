  ...
  <d2-container>
    <div style="width: 100%; margin: 0px auto;">
      ...
      >
        <el-table-column type="expand">
          <template slot-scope="props">
            <el-form label-position="left" inline class="demo-table-expand">
              ...
              <el-form
                v-for="(item) in props.row.fields"
                :key="item.id"
                size="small"
                :inline="true"
                label-width="60px"
        ...
        <el-table-column label="表名" prop="name" sortable></el-table-column>
        <el-table-column label="别名" prop="alias" sortable></el-table-column>
        <el-table-column label="创建人" prop="creator_username" sortable></el-table-column>
        <el-table-column label="创建时间" prop="creation_time" sortable></el-table-column>
        <el-table-column label="操作">
          <template slot-scope="scope">
            <el-button
              ...
              type="info"
              style="padding: 4px"
              @click="on_show_detail(scope.row)"
            >详情</el-button>
            <el-button
              size="mini"
              type="warning"
              style="padding: 4px"
              @click="on_change(scope.row)"
            >修改</el-button>
            <el-button
              size="mini"
              type="danger"
              style="padding: 4px"
              @click="on_delete(scope.row)"
            >删除</el-button>
          </template>
        </el-table-column>
      </el-table>
...
export default {
  ...
  mounted () {
    Masters.get(`mgmt/table`, { params: { page: 1, size: 10 } })
      .then(response => {
  ...
  methods: {
    ...
    on_delete (table) {
      ...
      this.$confirm(
        `删除表将删除表中所有数据，包括修改记录和已删除记录，是否继续？`,
        `警告`,
        { type: `warning` }
      ).then(() => {
        var formData = JSON.parse(JSON.stringify(table))
        Masters.delete(`mgmt/table/${formData.id}`)
          .then(response => {
    ...
    on_add_table () {
      var Comp = Vue.component(`ChangeTable`, ChangeTable)
      var vm = new Comp({
        propsData: { is_create: true, id: this.tables.length }
      })
